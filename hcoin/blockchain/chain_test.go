...
func TestBlockchain(t *testing.T) {
	...
	t.Run("Should restore blockchain", func(t *testing.T) {
		...
		dbStorage = fakeDB{
			fakeLoadChain: func() []byte {
				bc := &blockchain{Height: 2, NewestHash: "x", CurrentDifficulty: 1}
...
func TestAddPeerBlock(t *testing.T) {
	bc := &blockchain{Height: 1, CurrentDifficulty: 1, NewestHash: "x"}
	m.Txs["test"] = &Tx{}
	nb := &Block{Difficulty: 2, Hash: "test", Transactions: []*Tx{
		{ID: "test"},
	}}
	bc.AddPeerBlock(nb)
	if bc.CurrentDifficulty != 2 || bc.Height != 2 || bc.NewestHash != "test" {
		t.Error("AddPeerBlockl() should mutate the blockchain")
	}
}
